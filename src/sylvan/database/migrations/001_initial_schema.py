...
async def up(backend: StorageBackend, dialect: Dialect) -> None:
    ...
    await schema.create("schema_version", lambda t: (
    ...
    await schema.create("blobs", lambda t: (
    ...
    await schema.create("sections", lambda t: (
        t.id(),
        t.foreign_id("file_id", table="files"),
    ...
    await schema.create("file_imports", lambda t: (
        t.id(),
        t.foreign_id("file_id", table="files"),
        t.text("specifier"),
    ...
    await schema.create("usage_stats", lambda t: (
        t.foreign_id("repo_id"),
        t.text("date"),
    ...
    await schema.create("quality", lambda t: (
    ...
    await schema.create("blame_cache", lambda t: (
        t.integer("file_id"),
    ...
    await schema.create("workspaces", lambda t: (
        t.id(),
        t.text("name").unique(),
    ...
    await schema.create("coding_sessions", lambda t: (
    ...
    await schema.create("instances", lambda t: (
...
async def down(backend: StorageBackend, dialect: Dialect) -> None:
    ...
    for table in [
        "instances", "coding_sessions",
        "workspace_repos", "workspaces",
        "usage_stats", "blame_cache", "quality",
        "file_imports", "sections", "symbols",
        "blobs", "files", "repos", "schema_version",
    ]:
