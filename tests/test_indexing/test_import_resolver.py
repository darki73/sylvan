...
class TestPythonCandidates:
    def test_dotted_specifier_resolves_to_py_file(self):
        candidates = _generate_candidates("sylvan.search.embeddings", "python", "src/main.py", _CTX)
        assert "src/sylvan/search/embeddings.py" in candidates

    def test_dotted_specifier_resolves_to_init(self):
        candidates = _generate_candidates("sylvan.database.orm", "python", "src/main.py", _CTX)
        assert "src/sylvan/database/orm/__init__.py" in candidates

    def test_bare_import_generates_namespace_candidates(self):
        candidates = _generate_candidates("json", "python", "src/main.py", _CTX)
    ...
    def test_bare_import_os_generates_namespace_candidates(self):
        candidates = _generate_candidates("os", "python", "src/main.py", _CTX)
        assert "os/__init__.py" in candidates
        assert "os.py" in candidates

    def test_relative_single_dot(self):
        candidates = _generate_candidates(".utils", "python", "src/sylvan/search/main.py", _CTX)
        assert "src/sylvan/search/utils.py" in candidates

    def test_relative_double_dot(self):
        candidates = _generate_candidates("..config", "python", "src/sylvan/search/main.py", _CTX)
        assert "src/sylvan/config.py" in candidates

    def test_relative_dot_only(self):
        candidates = _generate_candidates(".", "python", "src/sylvan/search/main.py", _CTX)
...
class TestJsCandidates:
    def test_relative_import_without_extension(self):
        candidates = _generate_candidates("./utils", "javascript", "src/app.js", _CTX)
        assert "src/utils.js" in candidates
        assert "src/utils.ts" in candidates

    def test_relative_import_with_extension(self):
        candidates = _generate_candidates("./utils.js", "javascript", "src/app.js", _CTX)
        assert "src/utils.js" in candidates

    def test_index_file_candidates(self):
        candidates = _generate_candidates("./components", "typescript", "src/app.ts", _CTX)
    ...
    def test_bare_specifier_not_resolved(self):
        candidates = _generate_candidates("react", "javascript", "src/app.js", _CTX)
        assert candidates == []

    def test_scoped_package_not_resolved(self):
        candidates = _generate_candidates("@angular/core", "typescript", "src/app.ts", _CTX)
        assert candidates == []

    def test_parent_directory(self):
        candidates = _generate_candidates("../config", "javascript", "src/components/app.js", _CTX)
        assert "src/config.js" in candidates

    def test_vue_file_import(self):
        candidates = _generate_candidates("./App.vue", "typescript", "src/main.ts", _CTX)
        assert "src/App.vue" in candidates

    def test_tsx_extension(self):
        candidates = _generate_candidates("./Button", "tsx", "src/components/Form.tsx", _CTX)
        assert "src/components/Button.tsx" in candidates


class TestGoCandidates:
    def test_stdlib_not_resolved(self):
        candidates = _generate_candidates("fmt", "go", "main.go", _CTX)
        assert candidates == []

    def test_stdlib_nested_not_resolved(self):
        candidates = _generate_candidates("net/http", "go", "main.go", _CTX)
        assert candidates == []

    def test_third_party_generates_suffix_candidates(self):
        candidates = _generate_candidates("github.com/org/repo/pkg/util", "go", "main.go", _CTX)
...
class TestRustCandidates:
    def test_std_not_resolved(self):
        candidates = _generate_candidates("std::collections::HashMap", "rust", "src/main.rs", _CTX)
        assert candidates == []

    def test_crate_specifier(self):
        candidates = _generate_candidates("crate::module::item", "rust", "src/main.rs", _CTX)
        assert "src/module.rs" in candidates
        assert "src/module/mod.rs" in candidates

    def test_crate_single_segment(self):
        candidates = _generate_candidates("crate::config", "rust", "src/main.rs", _CTX)
        assert "src/config.rs" in candidates


class TestJavaCandidates:
    def test_java_import(self):
        candidates = _generate_candidates("com.example.util", "java", "Main.java", _CTX)
        assert "com/example/util.java" in candidates

    def test_kotlin_import(self):
        candidates = _generate_candidates("com.example.util", "kotlin", "Main.kt", _CTX)
...
class TestCCandidates:
    def test_system_header_not_resolved(self):
        candidates = _generate_candidates("stdio.h", "c", "src/main.c", _CTX)
        assert candidates == []

    def test_quoted_include_relative(self):
        candidates = _generate_candidates("myheader.h", "c", "src/main.c", _CTX)
        assert "src/myheader.h" in candidates

    def test_project_relative(self):
        candidates = _generate_candidates("utils/helpers.h", "cpp", "src/main.cpp", _CTX)
...
class TestRubyCandidates:
    def test_relative_require(self):
        candidates = _generate_candidates("./lib/foo", "ruby", "app/main.rb", _CTX)
        assert "app/lib/foo.rb" in candidates

    def test_absolute_style_require(self):
        candidates = _generate_candidates("models/user", "ruby", "app/main.rb", _CTX)
...
class TestPhpCandidates:
    def test_namespace_import(self):
        candidates = _generate_candidates("App\\Models\\User", "php", "index.php", _CTX)
        assert "App/Models/User.php" in candidates

    def test_psr4_resolves_correctly(self):
        ctx = ResolverContext(psr4_mappings={"App\\": ["app/"]})
        candidates = _generate_candidates("App\\Models\\User", "php", "index.php", ctx)
    ...
    def test_psr4_multiple_directories(self):
        ctx = ResolverContext(psr4_mappings={"App\\": ["app/", "src/App/"]})
        candidates = _generate_candidates("App\\Models\\User", "php", "index.php", ctx)
        assert "app/Models/User.php" in candidates
        assert "src/App/Models/User.php" in candidates

    def test_psr4_no_match_falls_back(self):
        ctx = ResolverContext(psr4_mappings={"Vendor\\": ["vendor/src/"]})
        candidates = _generate_candidates("App\\Models\\User", "php", "index.php", ctx)
        assert "App/Models/User.php" in candidates

    def test_psr4_dev_autoload(self):
        ctx = ResolverContext(psr4_mappings={"Tests\\": ["tests/"]})
        candidates = _generate_candidates("Tests\\Feature\\ExampleTest", "php", "index.php", ctx)
    ...
    def test_psr4_deep_namespace(self):
        ...
        candidates = _generate_candidates("Database\\Factories\\UserFactory", "php", "index.php", ctx)
        assert candidates[0] == "database/factories/UserFactory.php"


class TestJsTsconfigAliases:
    def test_at_alias_resolves(self):
        ctx = ResolverContext(tsconfig_aliases={"@": ["resources/js"]})
        candidates = _generate_candidates("@/components/ui/button", "typescript", "src/app.tsx", ctx)
    ...
    def test_tilde_alias_resolves(self):
        ...
        candidates = _generate_candidates("~/utils/helpers", "tsx", "pages/index.tsx", ctx)
        assert "src/utils/helpers.ts" in candidates

    def test_sveltekit_lib_alias(self):
        ctx = ResolverContext(tsconfig_aliases={"$lib": ["src/lib"]})
        candidates = _generate_candidates("$lib/server/db", "typescript", "src/routes/+page.ts", ctx)
        assert "src/lib/server/db.ts" in candidates

    def test_no_alias_match_falls_through(self):
        ctx = ResolverContext(tsconfig_aliases={"@": ["resources/js"]})
        candidates = _generate_candidates("react", "typescript", "src/app.tsx", ctx)
        assert candidates == []

    def test_relative_import_unaffected(self):
        ctx = ResolverContext(tsconfig_aliases={"@": ["resources/js"]})
        candidates = _generate_candidates("./utils", "typescript", "src/app.ts", ctx)
    ...
    def test_longest_alias_wins(self):
        ...
        candidates = _generate_candidates("@/components/button", "tsx", "src/app.tsx", ctx)
        assert candidates[0] == "resources/js/components/custom/button"

    def test_vue_extension_generated(self):
        ctx = ResolverContext(tsconfig_aliases={"@": ["src"]})
        candidates = _generate_candidates("@/components/Header", "typescript", "src/app.ts", ctx)
        assert "src/components/Header.vue" in candidates


class TestCSharpCandidates:
    def test_namespace_import(self):
        candidates = _generate_candidates("MyApp.Models", "c_sharp", "Program.cs", _CTX)
        assert "MyApp/Models.cs" in candidates


class TestUnknownLanguage:
    def test_returns_empty(self):
        candidates = _generate_candidates("something", "brainfuck", "main.bf", _CTX)
        assert candidates == []
...
class TestMultipleCandidatesPicksFirst:
    def test_first_match_wins(self):
        ...
        candidates = _generate_candidates("sylvan.search.embeddings", "python", "main.py", _CTX)
